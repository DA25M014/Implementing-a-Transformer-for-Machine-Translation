...
def beam_search_decode(
    ...
    """
    Beam search decoding with Google NMT-style length normalisation.

    Maintains beam_size partial hypotheses, expanded one token at a time.
    Each hypothesis is scored by sum-of-log-probs divided by ((5+len)/(5+1))^alpha
    (Wu et al. 2016) -- penalises overly short outputs that greedy tends
    to terminate prematurely.

    Args:
        model         : trained Transformer (caller already called .eval())
        src           : [1, src_len] source token ids
        src_mask      : [1, 1, 1, src_len]
        max_len       : hard cap on generated length including <sos>
        start_symbol  : <sos> id
        end_symbol    : <eos> id
        pad_idx       : <pad> id (for tgt_mask construction)
        beam_size     : number of beams to keep
        length_penalty: alpha in (5+len)/6 ^ alpha; 0.6 is the GNMT default

    Returns:
        [1, out_len] including start_symbol and -- if reached -- end_symbol
        for the best-scoring completed hypothesis.
    """
    import math
    model = model.to(device)
    src      = src.to(device)
    ...
    # Each beam: (cumulative_logprob, token_id_tensor, finished_flag)
    ...
    for _ in range(max_len - 1):
        ...
        for lp, ys, _ in active:
            ...
            log_probs   = torch.log_softmax(next_logits, dim=-1) # [1, V]

            # Take the top beam_size next tokens
            top_lp, top_idx = log_probs.topk(beam_size, dim=-1)  # [1, beam_size]
        ...
        # Move newly-finished beams aside so we do not keep expanding them
        finished_beams.extend([(lp, ys, fin) for lp, ys, fin in beams if fin])
        beams = [(lp, ys, fin) for lp, ys, fin in beams if not fin]
        if not beams:
            break

    # If nothing finished cleanly, fall back to whatever the active beams are
    candidates = finished_beams if finished_beams else beams
    if not candidates:
        # Degenerate case: return just <sos>
